    ...
    @if ($isOpen)
        <div class="modal-overlay flex fixed inset-0 z-[1400] items-center justify-center p-4 bg-black/40" wire:keydown.escape="close">
            <div class="bg-white rounded-card max-w-[720px] w-full max-h-[85vh] overflow-y-auto shadow-xl">
                ...
                <div class="p-6">
                    {{-- Tab: Unggah Berkas --}}
                    <div @if ($activeTab !== 'upload') style="display:none" @endif>
                        <label class="flex flex-col items-center justify-center gap-2 border border-dashed border-[#CDD3DF] rounded-lg py-10 cursor-pointer hover:bg-[#F8F9FB] transition-colors mb-4">
                            ...
                            <span class="text-[11px] text-[#848CA3]">JPG, PNG, WEBP hingga 10MB</span>
                            <input type="file" accept="image/*" class="hidden" wire:model="newFile">
                        </label>

                        @if ($newFile)
                            <div class="flex items-center gap-3 mb-4 bg-[#F8F9FB] border border-[#E4E8EF] rounded-lg p-3">
                                <img src="{{ $newFile->temporaryUrl() }}" class="w-12 h-12 rounded-lg object-cover shrink-0" alt="Pratinjau">
                                <div class="min-w-0 text-[13px] font-semibold truncate">{{ $newFile->getClientOriginalName() }}</div>
                            </div>
                        @endif
